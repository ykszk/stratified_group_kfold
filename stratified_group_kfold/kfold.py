...
class StratifiedGroupKFold:
   ...
   '''
    ...
    def split(self, X, labels, groups):
        ...
        label_counts_per_fold = np.zeros((self.n_splits, num_labels),
                                         dtype=int)
        groups_per_fold = [set() for _ in range(self.n_splits)]

        def eval_label_counts_per_fold(y_counts, fold):
            fold += y_counts
            std_per_label = np.std(label_counts_per_fold,
                                   axis=0) / global_label_dist
            fold -= y_counts
        ...
        if self.shuffle:
            ...
            groups_and_label_counts.sort(
                key=lambda g_counts: -np.std(g_counts[1]) + rng.gauss(
                    0, mean_std))  # add rng.gauss to increase the randomness
        else:
            groups_and_label_counts.sort(
        ...
        for g, label_counts in groups_and_label_counts:
            evals = [
                eval_label_counts_per_fold(label_counts,
                                           label_counts_per_fold[i])
